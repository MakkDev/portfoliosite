...
export default function Navbar(props) {
  const executeExperienceScroll = () =>
    props.refProp4.current.scrollIntoView({ behavior: "smooth" });
  const executeSkillsScroll = () =>
    props.refProp.current.scrollIntoView({ behavior: "smooth" });
  const executeProjectsScroll = () =>
    props.refProp2.current.scrollIntoView({ behavior: "smooth" });
  const executeContactScroll = () =>
    props.refProp3.current.scrollIntoView({ behavior: "smooth" });
  ...
  return (
    <AppBar
      sx={{
        backgroundImage: "linear-gradient(to left, #660099, #8C40B3, #B380CC )",
      }}
      position="sticky"
    >
      <Toolbar>
        ...
        ></Typography>
        <Button
          size={largeAndUp ? "large" : isSmallScreen ? "small" : "medium"}
          sx={{
            fontFamily: "Montserrat",
            fontWeight: "600",
            mr: "2%",
